...
class CrossValidationLinceDM(LinceDM):
    def __init__(
        ...
        self.data_map = {
            "lince": {
                "train": [r"C:\Users\prabh\Downloads\codemix-898edb33edf13d1fa5568aa579aa5b2eb1c08a55\codemix-898edb33edf13d1fa5568aa579aa5b2eb1c08a55\data\data\lince\ner\train.json"],
                "validation": [r"C:\Users\prabh\Downloads\codemix-898edb33edf13d1fa5568aa579aa5b2eb1c08a55\codemix-898edb33edf13d1fa5568aa579aa5b2eb1c08a55\data\data\lince\ner\val.json"]
            }
        }
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if k < 0 or k >= num_splits:
            raise ValueError(f"Invalid value for k: {k}. Must be between 0 and {num_splits - 1}.")

        kf = KFold(n_splits=num_splits, shuffle=True, random_state=self.hparams.split_seed)

        splits = list(kf.split(np.zeros(self.dataset['train'].num_rows)))
        print(f"Number of splits available: {splits[k]}")
        train_idxs, val_idxs = splits[k]
